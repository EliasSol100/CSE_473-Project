...
$requestedEventId = isset($_GET['event']) ? trim((string) $_GET['event']) : events_default_selected_event_id();
...
$hasEvents = $events !== [];
$eventsSyncUrl = 'api/events_summary.php?event=' . rawurlencode($selectedEventId !== '' ? $selectedEventId : events_default_selected_event_id());
?>
<div class="container" data-live-collector-url="api/collect_live.php" data-live-events-url="<?= h($eventsSyncUrl) ?>" data-live-collector-interval="<?= h((string) $collectorIntervalMs) ?>">
    <div class="section-title">
        <div>
            <h2>Events outlook</h2>
            <p>Real Sydney events across the next three Sydney calendar days, paired with a forecast for every tracked parking facility in the live API network.</p>
        </div>
    ...
    <section class="empty-state card" data-events-empty<?= $hasEvents ? ' hidden' : '' ?>>No researched Sydney events are available for the current three-day forecast window yet.</section>
    ...
    <div data-events-content<?= $hasEvents ? '' : ' hidden' ?>>
        ...
        <section class="grid-two" style="margin-bottom:24px;" data-events-cards>
            <?php foreach ($events as $event): ?>
                ...
                <article class="panel event-selector<?= $active ? ' active' : '' ?>">
                    ...
                    <div class="event-actions">
                        <a class="btn btn-primary" href="events.php?event=<?= urlencode($event['id']) ?>">Open forecast</a>
        ...
        <section class="table-card" data-facility-search-scope>
            <div class="section-title">
                <div>
                    <h2>All facility forecasts for the selected event</h2>
                    <p>Every row below is a prediction for a tracked facility, blending its stored occupancy pattern with a real event's expected parking pressure.</p>
                </div>
            </div>

            <div class="filters">
                <input class="search-bar" type="text" placeholder="Filter facilities in this event forecast..." data-facility-search>
                <div class="notice" style="padding:12px 14px;">
                    <h3 style="margin-bottom:4px;">Method</h3>
                    <p class="muted" style="margin:0;">Stored same-hour occupancy pattern + event attendance estimate + venue proximity and corridor weighting.</p>
                </div>
            </div>
            ...
            <div class="table-wrap">
                <table>
                    ...
                    <tbody data-events-table-body>
                        <?php foreach (($selectedEvent['impact_ranked'] ?? []) as $row): ?>
