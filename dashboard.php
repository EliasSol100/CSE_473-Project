<?php
// The dashboard brings together the current network totals, charts, and +1h/+2h/+3h forecasts.
$pageTitle = 'Dashboard';
require_once __DIR__ . '/includes/header.php';
require_once __DIR__ . '/includes/live_collector.php';

// Match the browser refresh timing with the live collector so the numbers stay recent.
...
// The same data is passed to JavaScript so cards and charts can update without a full page reload.
$dashboardPayload = [
    'summary' => $summary,
...
<div class="container" data-live-collector-url="api/collect_live.php" data-live-collector-interval="<?= h((string) $collectorIntervalMs) ?>" data-live-summary-url="api/live_summary.php">
    ...
    <section class="kpi-grid">
        ...
        <article class="card"><h3>Predicted Free (<?= h((string) ($predictionWindows['horizon_1h_label'] ?? '+1h')) ?>)</h3><div class="metric" data-prediction-1h-available><?= h(format_number($predictionSummary['horizon_1h_available_total'] ?? 0)) ?></div><p class="muted">Forecast available spaces for <?= h((string) ($predictionWindows['horizon_1h_window_label'] ?? 'the +1h window')) ?> in <?= h((string) ($predictionWindows['timezone'] ?? 'Australia/Sydney')) ?>.</p></article>
        <article class="card"><h3>Predicted Free (<?= h((string) ($predictionWindows['horizon_2h_label'] ?? '+2h')) ?>)</h3><div class="metric" data-prediction-2h-available><?= h(format_number($predictionSummary['horizon_2h_available_total'] ?? 0)) ?></div><p class="muted">Forecast available spaces for <?= h((string) ($predictionWindows['horizon_2h_window_label'] ?? 'the +2h window')) ?> across all tracked sites.</p></article>
        <article class="card"><h3>Predicted Free (<?= h((string) ($predictionWindows['horizon_3h_label'] ?? '+3h')) ?>)</h3><div class="metric" data-prediction-3h-available><?= h(format_number($predictionSummary['horizon_3h_available_total'] ?? 0)) ?></div><p class="muted">Forecast available spaces for <?= h((string) ($predictionWindows['horizon_3h_window_label'] ?? 'the +3h window')) ?> across the monitored network.</p></article>
...
<script>
// These arrays give Chart.js its first render; later updates come from the JSON API.
